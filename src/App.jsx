...
export default function IntervalTrainerPage() {
  ...
  const soundfontScriptPromiseRef = useRef(null);
  ...
  const [audioStatus, setAudioStatus] = useState("Los sonidos SoundFont se cargan al reproducir por primera vez.");
  ...
  const ensureSoundfontPlayer = useCallback(() => {
    if (typeof window === "undefined") return Promise.reject(new Error("SoundFont solo puede cargarse en el navegador"));
    if (window.Soundfont?.instrument) return Promise.resolve(window.Soundfont);
    if (soundfontScriptPromiseRef.current) return soundfontScriptPromiseRef.current;

    soundfontScriptPromiseRef.current = new Promise((resolve, reject) => {
      const existingScript = document.querySelector(`script[src="${SOUNDFONT_SCRIPT_URL}"]`);

      const resolveWhenReady = () => {
        if (window.Soundfont?.instrument) {
          resolve(window.Soundfont);
        } else {
          reject(new Error("soundfont-player se cargó, pero no expuso window.Soundfont"));
        }
      };

      if (existingScript) {
        existingScript.addEventListener("load", resolveWhenReady, { once: true });
        existingScript.addEventListener("error", () => reject(new Error("No se pudo cargar soundfont-player")), { once: true });
        return;
      }
      ...
      script.onerror = () => reject(new Error("No se pudo cargar soundfont-player"));
      document.head.appendChild(script);
    });

    return soundfontScriptPromiseRef.current;
  }, []);
    ...
    try {
      ...
      if (sampleInstrument) {
        ...
        exercise.sequence.forEach((note, index) => {
          const node = sampleInstrument.play(note.midi, baseTime + index * step, {
